...
function retricon(str, opts) {
	...
	var csize = (pixelSize * dimension) + (opts.imagePadding * 2) + Math.max(border, 0);
	var ctx = gm(csize, csize, 'transparent');
	
	
	if (_.isString(opts.bgColor)) {
		ctx.fill(opts.bgColor);
	} else if (_.isNumber(opts.bgColor)) {
		ctx.fill('#' + id.colors[opts.bgColor]);
	}
	
	if (! _.isNull(opts.bgColor))
		ctx.drawRectangle(0, 0, csize, csize);

	if (_.isString(opts.pixelColor)) {
		ctx.fill(opts.pixelColor);
	} else if (_.isNumber(opts.pixelColor)) {
		ctx.fill('#' + id.colors[opts.pixelColor]);
	} else {
		ctx.fill('white');
	}
	...
	for (var x=0; x<dimension; x++) {
		for (var y=0; y<dimension; y++) {
			if (pic[y][x]) {
				var x0 = (x*pixelSize) + border + opts.imagePadding,
					y0 = (y*pixelSize) + border + opts.imagePadding,
					width = pixelSize - (border * 2);

				ctx.drawRectangle(x0, y0, x0 + width, y0 + width);
			}
		}
	}
	return ctx;
